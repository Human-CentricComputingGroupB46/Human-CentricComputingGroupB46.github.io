...
const rooms: Room[] = [
  ...
  // --- South corridor rooms (west → east) ---
  ...
  // --- East corridor rooms (north → south) ---
...
// ---- Entrances (floor 2 access via stairs/elevators only) ----
...
const corridors: Corridor[] = [
  ...
  // South (inner) corridor — along EB222-EB206 rooms
  {
    id: 'F2_CORR_INNER',
    floor: 'floor2',
    path: [
  ...
  // West vertical connector: north corridor → south area
  {
    id: 'F2_CONN_W',
    floor: 'floor2',
    path: [
...
const nodes: GraphNode[] = [
  // Stair/elevator arrival points (treated as "entrances" on F2)
...
const edges: GraphEdge[] = [
  // NW arrival → north corridor
  edge('F2_NW', 'F2N_J1', nodes),
  edge('F2_NW', 'F2_ELEV_NW', nodes),

  // North corridor chain
  edge('F2N_J1', 'F2N_J2', nodes),
  edge('F2N_J2', 'F2N_J3', nodes),
  edge('F2N_J3', 'F2N_J4', nodes),
  edge('F2N_J4', 'F2N_J5', nodes),
  edge('F2N_J5', 'F2N_J6', nodes),
  edge('F2N_J6', 'F2N_J7', nodes),
  edge('F2N_J7', 'F2N_J8', nodes),
  edge('F2N_J8', 'F2N_J9', nodes),
  edge('F2N_J9', 'F2_NE', nodes),
  edge('F2N_J9', 'F2_STAIR_NE', nodes),

  // North corridor room doors
  edge('F2N_J2', 'D_EB237', nodes),
  edge('F2N_J2', 'D_EB239', nodes),
  edge('F2N_J3', 'D_EB233', nodes),
  edge('F2N_J3', 'D_EB235', nodes),
  edge('F2N_J4', 'D_EB231', nodes),
  edge('F2N_J6', 'D_EB211', nodes),

  // Vertical connector: north → inner corridor (west)
  edge('F2N_J4', 'F2I_J1', nodes),

  // Inner corridor chain
  edge('F2I_J1', 'D_EB222', nodes),
  edge('D_EB222', 'F2I_J2', nodes),
  edge('F2I_J2', 'D_EB220', nodes),
  edge('F2I_J2', 'D_EB218', nodes),
  edge('F2I_J2', 'F2I_J3', nodes),
  edge('F2I_J3', 'D_EB216', nodes),
  edge('F2I_J3', 'D_EB214', nodes),
  edge('F2I_J3', 'F2I_J4', nodes),
  edge('F2I_J4', 'D_EB212', nodes),
  edge('F2I_J4', 'D_EB210', nodes),
  edge('F2I_J4', 'F2I_J5', nodes),
  edge('F2I_J5', 'D_EB206', nodes),
  edge('F2I_J5', 'F2I_J6', nodes),

  // East corridor: connects to north corridor
  edge('F2N_J8', 'F2I_J6', nodes),
  edge('F2I_J6', 'F2E_J1', nodes),
  edge('F2I_J6', 'D_EB241', nodes),
  edge('F2E_J1', 'D_EB245', nodes),
  edge('F2E_J1', 'D_EB247', nodes),
  edge('F2E_J1', 'F2E_J2', nodes),
  edge('F2E_J2', 'D_EB249', nodes),
  edge('F2E_J2', 'D_EB251', nodes),
  edge('F2E_J2', 'F2E_J3', nodes),
  edge('F2E_J3', 'D_EB253', nodes),
  edge('F2E_J3', 'D_EB257', nodes),
  edge('F2E_J3', 'F2E_J4', nodes),
  edge('F2E_J4', 'D_EB259', nodes),
  edge('F2E_J4', 'F2E_J5', nodes),
  edge('F2E_J5', 'D_EB261', nodes),
  edge('F2E_J5', 'F2_STAIR_SE', nodes),

  // South corridor: connects east corridor to SW
  edge('F2E_J4', 'F2S_J9', nodes),
  edge('F2S_J9', 'D_EB265', nodes),
  edge('F2S_J9', 'F2S_J8', nodes),
  edge('F2S_J8', 'D_EB265A', nodes),
  edge('F2S_J8', 'F2S_J7', nodes),
  edge('F2S_J7', 'D_EB269', nodes),
  edge('F2S_J7', 'F2S_J6', nodes),
  edge('F2S_J6', 'D_EB271', nodes),
  edge('F2S_J6', 'F2S_J5', nodes),
  edge('F2S_J5', 'D_EB273', nodes),
  edge('F2S_J5', 'F2S_J4', nodes),
  edge('F2S_J4', 'D_EB275', nodes),
  edge('F2S_J4', 'F2S_J3', nodes),
  edge('F2S_J3', 'D_EB277', nodes),
  edge('F2S_J3', 'D_EB279', nodes),
  edge('F2S_J3', 'F2S_J2', nodes),
  edge('F2S_J2', 'D_EB283', nodes),
  edge('F2S_J2', 'D_EB287', nodes),
  edge('F2S_J2', 'F2S_J1', nodes),
  edge('F2S_J1', 'D_EB282', nodes),
  edge('F2S_J1', 'D_EB280', nodes),
  edge('F2S_J1', 'F2_SW', nodes),
  edge('F2_SW', 'F2_ELEV_SW', nodes),

  // West vertical: north → west block → south
  edge('F2N_J1', 'F2W_J1', nodes),
  edge('F2W_J1', 'D_EB236', nodes),
  edge('F2W_J1', 'D_EB238', nodes),
  edge('F2W_J1', 'F2W_J2', nodes),
  edge('F2W_J2', 'F2S_J1', nodes),
];
